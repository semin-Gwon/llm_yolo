...
| 기능 | 상태 |
|---|---|
| Named Place 이동 (`center 로 가`) | ✅ 완료 |
| 객체 탐색 + Scan 회전 탐색 (`chair 찾아`) | ✅ 완료 |
| Ground-Truth Perception (Prim Path 기반) | ✅ 완료 |
| YOLO 기반 Perception (`chair`, `tv`) | ✅ 완료 |
| Ollama LLM 기반 자연어 해석 | ✅ 완료 |
| `mission_plan` 복합 명령 & 조건 실행 (`run_if`) | ✅ 완료 |
| `approach_object` — YOLO + Depth + TF 기반 객체 접근 | ✅ 완료 |
| `speed_hint` 속도 조절 (천천히 / 빠르게) | ✅ 완료 |
| 긴급 정지 / 정지 해제 (`emergency_stop`) | ✅ 완료 |
| Nav2 / Direct 이중 주행 경로 | ✅ 완료 |
| Person 검출 기반 Pause / Resume | ✅ 완료 |
